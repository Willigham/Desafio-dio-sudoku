...
public class MainScreen {
    ...
    private final static Dimension dimencion = new Dimension(600, 600);

    private final BoardService boardService;
    private final NotifierService notifieService;
    ...
    public MainScreen(final Map<String, String> gameConfig){
        ...
        this.notifieService = new NotifierService();
    }

    public void buildMainScreen(){
        JPanel mainPanel = new MainPanel(dimencion);
        JFrame mainFrame = new MainFrame(dimencion, mainPanel);
        for (int r =0; r < 9; r+=3){
            var endRow = r + 2;
            for (int c = 0; c < 9; c += 3){
                var endCol = c + 2;
    ...
    private JPanel genereteSection(final List<Space> spaces){
        ...
        fields.forEach(t -> notifieService.subscriber(CLEAR_SPACE, t));
    ...
    private void addResetButton(final JPanel mainPanel) {
        resetButton = new ResetButton(e -> {
            ...
            if (dialogResult == 0){
                boardService.reset();
                notifieService.notify(CLEAR_SPACE);
            }
        });
